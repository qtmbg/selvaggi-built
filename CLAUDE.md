...
- **Routes (hash-based):** `#/home`, `#/expertise`, `#/projects`, `#/company`,
  `#/how-we-work`, `#/insights`, `#/contact`, `#/accessibility`, `#/privacy`, `#/terms`.
  Vercel `cleanUrls` is on, so `/how-we-work` also resolves.
- **`api/ai.js`** — Vercel serverless function. Server-side proxy to Claude (Anthropic) for the
  RFP / ICRA / Estimator features. Holds the API key server-side; never expose it to the client.
- **`api/contact.js`** — Vercel serverless function. POSTs contact-form submissions to a CRM
  webhook. Returns 503 until `CRM_WEBHOOK_URL` is configured.
...
- `ANTHROPIC_API_KEY` — for `/api/ai`
- `ANTHROPIC_MODEL` — default `claude-sonnet-4-6`
- `CRM_WEBHOOK_URL` (+ optional `CRM_AUTH_HEADER`) — for `/api/contact`
